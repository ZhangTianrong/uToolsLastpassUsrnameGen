	...
	MobileSelect.prototype = {
		...
		addListenerWheel: function(theWheel, index){
			...
			if(_this.isPC){
				...
				theWheel.addEventListener('wheel', function() {
					_this.mouseWheel(event, this.firstChild, index)
				},true);
			}
		},
		...
		mouseWheel: function(event, theSlider, index){
			var _this = this;
			event = event || window.event;
			if (event.type == "wheel") {
				event.preventDefault();
				{
					_this.offset = -4 * Math.sign(event.deltaY)*Math.log(Math.abs(event.deltaY) + 1);
					_this.updateCurDistance(theSlider, index);
					_this.curDistance[index] = _this.curDistance[index] + _this.offset;
					_this.movePosition(theSlider, _this.curDistance[index]);
					_this.oldMoveY = _this.moveY;
				...
				if(_this.curDistance[index] + _this.offset > 2*_this.liHeight){
					_this.curDistance[index] = 2*_this.liHeight;
					setTimeout(function(){
						_this.movePosition(theSlider, _this.curDistance[index]);
					}, 100);
		
				}else if(_this.curDistance[index] + _this.offset < _this.oversizeBorder){
					_this.curDistance[index] = _this.oversizeBorder;
					setTimeout(function(){
						_this.movePosition(theSlider, _this.curDistance[index]);
					}, 100);
